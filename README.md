...
```bash
cd AnsibleTest
ansible-playbook install_setup_webserver.yaml
```
...
```bash
ansible-playbook install_setup_webserver.yaml --limit ansible-managed-node
ansible-playbook install_setup_webserver.yaml --check          # dry run
ansible-playbook install_setup_webserver.yaml --syntax-check    # validate without connecting
ansible-playbook install_setup_webserver.yaml -K                # if sudo needs a password prompt
```
...
All of steps 2-10 happen inside a single `block:` in `install_setup_webserver.yaml`; see
...
```bash
ansible-playbook install_setup_webserver.yaml --syntax-check
